...
module.exports = function (api) {
  ...
  return {
    ...
    plugins: [
      ({ types: t }) => ({
        name: "astrowalla-astrologer-expo-router-root",
        visitor: {
          MemberExpression(memberPath, state) {
            ...
            if (key.value !== "EXPO_ROUTER_APP_ROOT") return;
            const from = path.dirname(state.filename || __filename);
            memberPath.replaceWith(t.stringLiteral(path.relative(from, path.join(__dirname, "app"))));
          },
        },
      }),
    ],
